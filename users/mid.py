...
OLD_USER = "./users_old.csv"
OLD_QUEUE = "./queue_user_old.csv"
...
def getUsers():
    ...
    with open(OLD_QUEUE, "r") as f:
        ...
        try:
            for row in reader:
                candidateQue.put(row[0])
        except csv.Error as e:
            sys.exit('file {}, line {}: {}'.format(OLD_QUEUE, reader.line_num, e))
    with open(OLD_USER, "r") as f:
        reader = csv.reader(f)
        try:
            for row in reader:
                dict[row[0]] = True
        except csv.Error as e:
            sys.exit('file {}, line {}: {}'.format(OLD_USER, reader.line_num, e))
    while candidateQue.qsize() > 0:
        ...
        for friend in friends:
            if (not dict.has_key(friend)) and (not candidateQue.full()):
                ...
                print "queue size is " + str(candidateQue.qsize())
